...
omega_max = phi*1000/(t_r/2);
t_min = round(((2*pi-phi)/omega_max)*1000);
...
while i<qt_amostras
    ...
    aux=1;
    v_calc(tmp_v) = 2*pi*r/t_min*1000;
    ...
    if (v_sinal_r_comp(i) == vcc && flag_high==1 && flag_low==1)
        ...
        v_calc(tmp_v) = 2*pi*r/(f_periodo-i_periodo+t_r)*1000;
        erro(tmp_v) = (v_calc(tmp_v)/v_r(tmp_v)-1)*100;
