...
for linha=1:n_amostras,
    ...
    for coluna=1:n_bits_ad,
        if(v0(linha)>= vref_ad/2^(coluna)+tmp)
            ...
            tmp2 = tmp2 - (vref_ad/2^(coluna) );%desacumulador
        else
            vetor_quantizado(linha,coluna) = 0; %zera bit
            vetor_quantizado_c1(linha, coluna) = 1; %seta bit
        end
        vetor_quantizado_converted(linha)=tmp2;%salva o maior valor do desacumulador, número decimal convertido do complemento de 1
    end
