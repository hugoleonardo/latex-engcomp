...
for i=1:N
    % Dn é calculado, para  n=1,...,n=N, e armazenado em D(N+2),...,D(2N+1) 
    Func = @(t) exp (-j*2*pi*t*i/T).*funct_tri(t/2)
    D(i+N+1) = quad(Func,a,b,tol)
    % Dn pe calculado, para n = -N,...,-1 e armazenando em D(1),...,D(N)
...
subplot(2,1,1); s1=plot(intervalo,abs(D));
set(s1,'Linewidth',2); 
ylabel('|{\it D}_{\it n}|');
title('Amplitude de {\it D}_{\it n}');
subplot (2,1,2); s2=plot(intervalo,angle(D));
