...
vet_resultado_avg_aptidao_08(1,n_temperatura+1) = 0;
...
%alpha = 0.85; % cte de resfriamento
temperatura_congelamento = 0.00001; % valor fixo
...
% gera solução inicial (X) com as linhas certas
% linhas certas
%{
for i=1:n_lin
    if mat_lin(i,1) ~=0 && mat_lin(i,2) ~=0
        rand_aux = randi([1,(n_col)-(mat_lin(i,1))-(mat_lin(i,2))]);
        for j=1:(rand_aux)-1
            mat_temp(i,j) = 0;
        end
        for j=rand_aux:((rand_aux+(mat_lin(i,1)))-1)
            mat_temp(i,j) = 1;
        end
        indice = j;
        rand_aux = randi([indice+1,(n_col)-(mat_lin(i,2))]);
        for j=indice+1:rand_aux
            mat_temp(i,j) = 0;
        end
        indice = j;
        for j=indice+1:indice+(mat_lin(i,2))
            mat_temp(i,j) = 1;
        end
        for j=indice+(mat_lin(i,2))+1:n_col
            mat_temp(i,j) = 0;
        end
    elseif mat_lin(i,1) ~=0
        rand_aux = randi([1,(n_col)-(mat_lin(i,1))]);
        for j=1:(rand_aux)-1
            mat_temp(i,j) = 0;
        end
        for j=rand_aux:((rand_aux+(mat_lin(i,1)))-1)
        ...
        for j=(rand_aux+(mat_lin(i,1))):n_col
            mat_temp(i,j) = 0;
        end
    end 
end
    ...
    n_temperatura = n_temperatura + 1;
        ...
        %while (alpha < 1)
            ...
            while (t > temperatura_congelamento && custo_final~= 0.01)
                n_t = 1000; % número de iterações com a mesma temperatura
                while n_t > 0
                    % ================================================================
                    ...
                    %}
                    % escolhe a matriz vizinha alterando uma coluna errada e
                    % deixando-a certa
                    escolhe_linha_coluna = 0;
                    rand_escolhe = 0;
                    rand_escolhe = rand(1);
                    if rand_escolhe < 0.5
                        rand_aux = randi([1,(n_col)]);
                        indice_col = rand_aux;
                        for j=1:n_col
                            for i=1:n_lin
                                if j ~= indice_col
                                    ...
                                    if mat_col(indice_col,1) ~=0 && mat_col(indice_col,2) ~=0
                                        rand_aux = randi([1,(n_lin)-(mat_col(indice_col,1))-(mat_col(indice_col,2))]);
                                        for i=1:(rand_aux)-1
                                            mat_vizinho(i,indice_col) = 0;
                                        end
                                        for i=rand_aux:((rand_aux+(mat_col(indice_col,1)))-1)
                                            mat_vizinho(i,indice_col) = 1;
                                        end
                                        indice = i;
                                        rand_aux = randi([indice+1,(n_lin)-(mat_col(indice_col,2))]);
                                        for i=indice+1:rand_aux
                                            mat_vizinho(i,indice_col) = 0;
                                        end
                                        indice = i;
                                        for i=indice+1:indice+(mat_col(indice_col,2))
                                            mat_vizinho(i,indice_col) = 1;
                                        end
                                        for i=indice+(mat_col(indice_col,2))+1:n_lin
                                            mat_vizinho(i,indice_col) = 0;
                                        end
                                    elseif mat_col(indice_col,1) ~=0
                                        rand_aux = randi([1,(n_lin)-(mat_col(indice_col,1))]);
                                        for i=1:(rand_aux)-1
                                            mat_vizinho(i,indice_col) = 0;
                                        end
                                        for i=rand_aux:((rand_aux+(mat_col(indice_col,1)))-1)
                                        ...
                                        for i=(rand_aux+(mat_col(indice_col,1))):n_lin
                                            mat_vizinho(i,indice_col) = 0;
                                        end
                                    end
                                end
                        ...
                        for i=1:n_lin
                            for j=1:n_col
                                if i ~= indice_lin
                                    ...
                                    if mat_lin(indice_lin,1) ~=0 && mat_lin(indice_lin,2) ~=0
                                        rand_aux = randi([1,(n_col)-(mat_lin(indice_lin,1))-(mat_lin(indice_lin,2))]);
                                        for j=1:(rand_aux)-1
                                            mat_vizinho(indice_lin,j) = 0;
                                        end
                                        for j=rand_aux:((rand_aux+(mat_lin(indice_lin,1)))-1)
                                            mat_vizinho(indice_lin,j) = 1;
                                        end
                                        indice = j;
                                        rand_aux = randi([indice+1,(n_col)-(mat_lin(indice_lin,2))]);
                                        for j=indice+1:rand_aux
                                            mat_vizinho(indice_lin,j) = 0;
                                        end
                                        indice = j;
                                        for j=indice+1:indice+(mat_lin(indice_lin,2))
                                            mat_vizinho(indice_lin,j) = 1;
                                        end
                                        for j=indice+(mat_lin(indice_lin,2))+1:n_col
                                            mat_vizinho(indice_lin,j) = 0;
                                        end
                                    elseif mat_lin(indice_lin,1) ~=0
                                        rand_aux = randi([1,(n_col)-(mat_lin(indice_lin,1))]);
                                        for j=1:(rand_aux)-1
                                            mat_vizinho(indice_lin,j) = 0;
                                        end
                                        for j=rand_aux:((rand_aux+(mat_lin(indice_lin,1)))-1)
                                        ...
                                        for j=(rand_aux+(mat_lin(indice_lin,1))):n_col
                                            mat_vizinho(indice_lin,j) = 0;
                                        end
                                    end
                                end
            ...
            %alpha = alpha + cte_crescimento
        %end
        %t_inicial = 2^(n_temperatura)
        %{
        if alpha == 0.8000
            vet_resultado_avg_aptidao_08(1,n_temperatura) = mean(vet_resultado_aptidao);
        elseif alpha == 0.8500
            vet_resultado_avg_aptidao_085(1,n_temperatura) = mean(vet_resultado_aptidao);
        elseif alpha == 0.9000
            vet_resultado_avg_aptidao_09(1,n_temperatura) = mean(vet_resultado_aptidao);
        elseif alpha == 0.9500
            vet_resultado_avg_aptidao_095(1,n_temperatura) = mean(vet_resultado_aptidao);
        end
        %}
    end
...
title('alpha = 0.95, Solução inicial: aleatória, Vizinho: método 2');
xlabel('Temperatura inicial'); ylabel('Aptidão média para cada temperatura inicial');
%{
figure;
plot(vet_n_temperatura,vet_resultado_avg_aptidao_085);
title('alpha = 0.85');xlabel('temperatura'); ylabel('aptidão média ');
figure;
plot(vet_n_temperatura,vet_resultado_avg_aptidao_09);
title('alpha = 0.9');xlabel('temperatura'); ylabel('aptidão média ');
figure;
plot(vet_n_temperatura,vet_resultado_avg_aptidao_095);
title('alpha = 0.95');xlabel('temperatura'); ylabel('aptidão média ');
%}
